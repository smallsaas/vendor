...
`name` VARCHAR (50) not NULL COMMENT '影线名称',
`org_id` bigint(20) NOT NULL,
`create_time` datetime DEFAULT CURRENT_TIMESTAMP,
`update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
`note` text COMMENT '说明 备注',
PRIMARY KEY (`id`)
)ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='影线表';
...
`id` bigint(20) NOT NULL AUTO_INCREMENT,
`name` VARCHAR(60) not NULL COMMENT '影院名称',
`line_name` VARCHAR(60) not NULL COMMENT '影线名称',
`pcd_id` bigint(20) DEFAULT NULL COMMENT '所在省市的pcdid',
`ticket` decimal(11,2) DEFAULT 0 COMMENT '票房',
`province` VARCHAR(20) not NULL COMMENT '所在省',
`city` VARCHAR(20) not NULL COMMENT '所在城市',
`district` VARCHAR(20) DEFAULT NULL COMMENT '市区',
`area` varchar(60) DEFAULT NULL COMMENT '地区，例如：  华北地区',
`line_id` INT(10) DEFAULT NULL COMMENT '所属影线',
`qualification_number` VARCHAR (50) not NULL COMMENT '专资编号',
`room_number` INT(10) DEFAULT 0 COMMENT '影厅数',
`seat_number` INT(10) DEFAULT 0 COMMENT '总座位数',
`contact_name` varchar (20) not NULL COMMENT '联系人名字',
`contact` VARCHAR (20) DEFAULT NULL COMMENT '联系电话',
`city_level` VARCHAR (20) DEFAULT '一线' COMMENT '城市级别',
`assistant_id` BIGINT(20) DEFAULT NULL COMMENT '商务id',
`business_area` VARCHAR (30) DEFAULT NULL COMMENT '所属商圈',
...
`business_license_phone` VARCHAR(255) DEFAULT NULL COMMENT '营业执照照片',
 `import_note` varchar(60) DEFAULT 'NOT DETECTED' COMMENT '导入数据检查情况',
 `average_ticket_price` decimal(10,2) DEFAULT NULL COMMENT '平均票价',
